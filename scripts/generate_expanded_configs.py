"""Generate the expanded 30-prompt benchmark configs for multi-seed ablation.

Creates 4 conditions x 3 seeds = 12 config files, each loading all prompts
from the four suite JSON files and assigning per-prompt canonical seeds with
an offset for each trial seed.
"""
        ...
        "adapters": [
            {
                ...
                "weight": 0.8,
                "params": {
                    "style_prompt": "black and white manga screentone, cinematic line art"
                },
            }
        ],
    },
        ...
        "adapters": [
            {
                ...
                "params": {
                    ...
                    "adapter_name": "planogram_sd21_lora",
                    "adapter_scale": 0.8,
                },
            }
        ],
        ...
        "adapters": [
            {
                ...
                "params": {
                    ...
                    "adapter_name": "cartoon_sd21_lora",
                    "adapter_scale": 0.8,
                },
            }
        ],
...
def make_prompt_entry(
    prompt: dict, condition_key: str, trial_seed: int, trial_index: int
) -> dict:
    pid = prompt["id"]
    base_seed = BASE_SEED_MAP[pid]
    per_prompt_seed = base_seed + trial_seed
    ...
    entry = {
        ...
        "output_name": f"{pid}_{condition_key}_t{trial_index}",
        "guidance_scale": 6.0,
...
def generate_config(
    ...
    name = f"expanded_{condition_key}_t{trial_index}"
    return {
        "name": name,
        "description": f"Expanded 30-prompt ablation: {condition_def['label']}, trial {trial_index} (seed offset {trial_seed}).",
        "seed": trial_seed,
        "output_dir": "outputs",
        "model": MODEL_CONFIG,
    ...
    for condition_key, condition_def in CONDITIONS.items():
        for trial_index, trial_seed in enumerate(TRIAL_SEEDS):
            config = generate_config(
                condition_key, condition_def, trial_seed, trial_index, all_prompts
            )
            filename = f"expanded_{condition_key}_t{trial_index}.json"
